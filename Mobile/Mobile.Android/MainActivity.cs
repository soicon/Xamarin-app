            ...
            if (item.ItemId == 16908332) // xam forms nav bar back button id
            {
                try
                {
                 ...
                 MainPage.Navigation.NavigationStack.LastOrDefault();
                    if (currentpage.GetType() == typeof(Report))
                    {
                        var report = (Report)currentpage;
                        if (report?.CustomBackButtonAction != null)
                        {
                            report?.CustomBackButtonAction.Invoke();
                            return false;
                        }
                    }
                    else
